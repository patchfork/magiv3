...
class Florence2Seq2SeqLMOutput(ModelOutput):
    ...
    character_character_affinity_matrices: Optional[torch.FloatTensor] = None
    text_character_association_matrices: Optional[torch.FloatTensor] = None
    text_tail_association_matrices: Optional[torch.FloatTensor] = None
    essential_text_logits: Optional[torch.FloatTensor] = None
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    def __init__(self, config: Florence2Config):
        ...
        self.text_character_matching_head = nn.Sequential(
            nn.Linear(2 * config.projection_dim, config.projection_dim),
            nn.ReLU(),
            nn.Linear(config.projection_dim, config.projection_dim),
            nn.ReLU(),
            nn.Linear(config.projection_dim, 1)
        )
        self.text_tail_matching_head = nn.Sequential(
            nn.Linear(2 * config.projection_dim, config.projection_dim),
            nn.ReLU(),
            nn.Linear(config.projection_dim, config.projection_dim),
            nn.ReLU(),
    ...
    def predict_character_grounding(
        ...
        if not strict:
            ...
            generated_ids = self.slowly_generate_grounded_caption(
                input_ids=batch_inputs["input_ids"],
                pixel_values=batch_inputs["pixel_values"],
                captions=captions,
                processor=processor,
        ...
        return [
            {
                ...
                "bboxes": list_of_list_of_bboxes,
                "indices_of_bboxes_in_caption": indices_of_bboxes_in_generated_text,
            }
            for generated_text, list_of_list_of_bboxes, indices_of_bboxes_in_generated_text in zip(generated_texts, list_of_list_of_list_of_bboxes, batch_indices_of_bboxes_in_generated_text)
        ]
    ...
    def forward(
        ...
        return Florence2Seq2SeqLMOutput(
            ...
            text_character_association_matrices=text_character_association_matrices,
            text_tail_association_matrices=text_tail_association_matrices,
            essential_text_logits=essential_text_logits,
        )
    ...
    def slowly_generate_grounded_caption(
        ...
        **kwargs,
        ...
        """
        This function will guarantee that the grounded caption's tokens are exactly the same as the tokens in the caption (except for the <loc_*> tokens).
        Otherwise, the model may sometimes replaces input tokens such as "character" with "person" during generation. This is normally fine but becomes
        a problem during evaluation or if we need the grounded caption to be exactly the same as the caption.
        """
        ...
        while True:
            num_tokens_generated += 1
            # Generate a chunk of tokens (really just to leverage KV caching to some extent)
            predicted_next_tokens = self.language_model.generate(
                input_ids=None,
                inputs_embeds=inputs_embeds,
                decoder_input_ids=running_decoder_input_ids,
                max_new_tokens=CHUNK_SIZE+1,
                min_new_tokens=CHUNK_SIZE+1,
                use_cache=False,
                **kwargs,
            )[:, -(CHUNK_SIZE+1):-1]
            ...
            for i, (pnts, pnt) in enumerate(zip(predicted_next_token_strings, predicted_next_tokens)):
                if (pnts.startswith("<loc_") or pnts in ["<s>", "<pad>", "</s>"]) and running_indices[i] < decoder_input_ids.shape[1] - 1:
                    next_tokens_to_concat.append(pnt)
                else:
                    running_indices[i] += 1
